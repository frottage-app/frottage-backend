...
CREATE TABLE target (
  ...
  CHECK(
    aspect_ratio like '%:%' and
    cast(cast(substr(aspect_ratio, 1, instr(aspect_ratio, ':')-1) as integer) as text) = substr(aspect_ratio, 1, instr(aspect_ratio, ':')-1) and
    cast(cast(substr(aspect_ratio, instr(aspect_ratio, ':')+1) as integer) as text) = substr(aspect_ratio, instr(aspect_ratio, ':')+1)
  )
);
